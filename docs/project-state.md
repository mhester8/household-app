...
- **Realtime channel recovery restores delivery of future events only — it does not replay mutations missed while a channel was down.** Groceries is the one page that additionally guards against this: on a genuine `document.visibilitychange` hidden → visible transition, it re-fetches the authoritative `grocery_items` snapshot and reconciles it into local state (`lib/groceryReconciliation.ts`), protecting known in-flight local state (an optimistic add not yet confirmed by Supabase, an item mid-undo-window in `pendingDeletesRef`) so the fetch can't drop or resurrect the wrong row. This is Groceries-only, not a generalized capability of `lib/useRealtimeSubscription.ts` or applied to Recipes/Notes/This Week — those pages still rely solely on the recovery behavior described above. Groceries also exposes this same reconciliation as a manual pull-to-refresh gesture. Pull-to-refresh itself is a shared mechanism, not a Groceries-only feature: a single gesture listener and top indicator are mounted once by `PullToRefreshProvider` (`lib/PullToRefreshContext.tsx`) in `app/(app)/layout.tsx`, and any page can opt in via `usePullToRefreshHandler(refreshFn)` — a page that doesn't call it simply has the gesture disabled. Groceries registers `reconcileFromServer` (the same function the resume path above uses), and Workouts, Recipes, Notes, and This Week each register their own existing list-load function (`loadData`/`loadRecipes`/`loadNotes`/`loadQueue`, each hoisted out of its mount effect so it can be called from both places). Recipe detail/edit/import, individual notes, Pinterest, and workout sessions still don't participate. It's a convenience/fallback trigger, not a change to Realtime recovery itself, and has not yet been validated on a real device.
- **Pull-to-refresh convention: register, don't reimplement.** The gesture/indicator infrastructure above is shared and global — a new list/module should call `usePullToRefreshHandler(...)` with its existing safe reload/reconciliation function rather than building any new gesture handling. Don't enable it on every route by default; detail/edit/form-heavy pages should only opt in once re-fetching there can't disrupt meaningful in-progress local state (an open draft, unsaved form input, etc.).
